...
load('PaviaU.mat');
load('R_pavia.mat');
F=double(F);   F=F/max(F(:));
data.F = F(1:480, 1:320, 11:end);
R = R(:, 1:end-10);
clear F;      
szF=size(data.F);
...
%%  HSRSV
clearvars -except par data flag Tab TabH TabM SNRm SNRh kernel_type sigmah sigmam szF;
fprintf('==================================== HSRSV ==================================== \n');
par.K=5;        
par.iter=80;   
p={1/2,2/3};
...
Time.CSD_BTF=tic;
    [CSD_BTF, par] = CSD_BTF_Pavia(data, par, flag);   
Time.CSD_BTF=toc(Time.CSD_BTF);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(data.F)), double(im2uint8(CSD_BTF)), 0, 1.0/par.sf);
...
BGS.rho=1e-3;  
BGS.nu=1.3;    BGS.lambda=1e1;    BGS.mu=1e0;    BGS.addatoms=[0,0];
BGS.turank=[480,320,12];    BGS.shape_re=[15,32,10,32,3,4];
BGS.MaxLoop=500;    BGS.IniLoop=1000;
BGS.ConvTol=1e-7;
Time.BGS=tic;
    BGS.HSR=ReTF(H, M, BGS.Pr, BGS.lambda, BGS.mu, BGS.rho, BGS.nu, BGS.turank, BGS.addatoms, BGS.shape_re, BGS.IniLoop, BGS.MaxLoop, BGS.ConvTol);
Time.BGS=toc(Time.BGS);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(data.F)), double(im2uint8(BGS.HSR)), 0, 1.0/par.sf);
...
FuVarH = reshape(FuVarH',M1,M2,L);   

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
        Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(data.F)), double(im2uint8(FuVarH)), 0, 1.0/sf);
