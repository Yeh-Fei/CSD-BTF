...
SparseX={'1','mcp'};
SmoothA={'0','1'};
flag.Sum2One=0;  flag.SparseX=SparseX{2};  flag.SparseA=SmoothA{1};  flag.p=p{2};
Time.CSD_BTF=tic;
    [CSD_BTF, par] = CSD_BTF_Synthesized(data, par, flag);
Time.CSD_BTF=toc(Time.CSD_BTF);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(data.Fh)), double(im2uint8(CSD_BTF.Fh)), 0, 1.0/par.sf);
expr=['Tab.', 'CSD_BTF',  ' =[Eval.PSNR; Eval.RMSE; Eval.ERGAS; Eval.SAM; Eval.UIQI; Eval.SSIM; Eval.DD; Eval.CC; Time.CSD_BTF];'];
eval(expr);
disp(Tab);

%%  Variable
Fh=data.Fh;  Fm=data.Fm;  H=data.H;  M=data.M;  upH=data.upH;  R=par.R;  sf=par.sf;  psf=par.psf;  otf=par.otf;  shift=par.shift;
p=sqrt(diag(psf));
...
BGS.rho=1e-7;  
BGS.nu=1.2;    BGS.lambda=1e0;    BGS.mu=1e0;    BGS.addatoms=[0,0];
BGS.turank=[100,100,30];    BGS.shape_re=[4,25,4,25,2,15];
BGS.MaxLoop=500;    BGS.IniLoop=500;
BGS.ConvTol=1e-3;
Time.BGS=tic;
    BGS.HSR=ReTF_mod(H, M, BGS.Pr, BGS.lambda, BGS.mu, BGS.rho, BGS.nu, BGS.turank, BGS.addatoms, BGS.shape_re, BGS.IniLoop, BGS.MaxLoop, BGS.ConvTol);
Time.BGS=toc(Time.BGS);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(Fh)), double(im2uint8(BGS.HSR)), 0, 1.0/par.sf);
...
[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
        Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(Fh)), double(im2uint8(FuVarH)), 0, 1.0/sf);
...
GSFus.mu=1e-1;       
Time.GSFus=tic;
    GSFus.HSR = GSFus_main( H, M, R, otf, sf, Fh, GSFus.subspace, GSFus.lambda_1, GSFus.lambda_2, GSFus.mu);
Time.GSFus=toc(Time.GSFus);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(Fh)), double(im2uint8(GSFus.HSR)), 0, 1.0/sf);
...
Time.BTD_Var=tic;
    ...
    [A_hat,B_hat,S,C_hat,C_tilde,cost,valid] = BTD_Var_main(Fh,H,M,P1,P2,R,BTD_Var.R,BTD_Var.B0,BTD_Var.C0,R*BTD_Var.C0,BTD_Var.nIter,BTD_Var.lamda);
    BTD_Var.HSR = ll1gen({A_hat,B_hat,C_hat},BTD_Var.r);
Time.BTD_Var=toc(Time.BTD_Var);

[Eval.PSNR, Eval.RMSE, Eval.ERGAS, Eval.SAM, Eval.UIQI, Eval.SSIM, ...
     Eval.DD, Eval.CC] = quality_assessment(double(im2uint8(Fh)), double(im2uint8(BTD_Var.HSR)), 0, 1.0/sf);
